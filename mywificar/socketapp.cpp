...
extern NetworkPackage  mNetworkPack;

void ParseNetworkPack()
{
  char temp[32];
  int  i, dataIndex;
  
  i = 0;
  while((message[i] != '?')&&(i<32))
  {
    temp[i]=message[i];
...
//    Serial.print(temp[i]);
  ...
  if(i>32)
  {
    mNetworkPack.dataLength = 0;
    mNetworkPack.data[0] = 0;
    return;
  }
    
  temp[i]='\0';
//  temp[i+1]='\n';
  mNetworkPack.dataLength = atoi(temp);
  dataIndex = i+1;
//  Serial.print("dataIndex=");
//  Serial.println(dataIndex);

  for(i=0; i<mNetworkPack.dataLength; i++)
  {
   mNetworkPack.data[i] = message[dataIndex+i]; 
  }
  mNetworkPack.data[i]='\0';
...
void socket_app_appcall(void)
{
    ...
    */
   struct socket_app_state *s = (socket_app_state *)&(uip_conn->appstate);
...
//      sms[3] = s->inputbuffer[3];
      memcpy(message, s->inputbuffer, sizeof(s->inputbuffer));
